...
test('create General Admission section #ga', function(){
  ...
  deepEqual($('#ga .seat').length, 40, 'checking that 40 seats created under #ga');
  //--#ga divs have border
  ok(_.every($('#ga div').css('border'), function(item) {return item === true;}), 'check that all ga divs have a border');
  ok(_.every($('#ga > .seatRow'), function(row) {return $(row).children('div').length <= 10;}), 'check that each ga row only 10 seats wide');
});